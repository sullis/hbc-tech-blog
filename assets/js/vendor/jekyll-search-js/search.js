...
class jekyllSearch {
    ...
    async findResults() {
        ...
        return data.filter(item => {
            return item.title.match(regex) || item.excerpt.match(regex) || item.tags.match(regex) || item.category.match(regex) || item.author.match(regex);
        });
    }
        ...
        const html = results.map(item => {
            ...
            <section class="snippet snippet--reveal">
                <div class="snippet__meta">
                    <a class="meta__category-link" href="https://tech.hbc.com/category/${item.category}">${item.category}</a>
                    <span class="slug-divider"></span>
                    <span class="meta__author">${item.author}</span>
                    <span class="slug-divider"></span>
                    <span class="meta__date">${item.date}</span>
                </div>
                <h1 class="snippet__title"><a class="snippet__title__link" title="${item.title}" href="https://tech.hbc.com${item.url}">${item.title}</a></h1>
                <a class="snippet__excerpt__link" href="https://tech.hbc.com/${item.url}"><p class="snippet__excerpt">${item.excerpt}</p></a>
            </section>`;
        }).join('');
